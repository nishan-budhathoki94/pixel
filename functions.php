...
function pixel_setup() {
    ...
    if (function_exists('register_nav_menu')) :
        global $pixel_options;
        register_nav_menu('primary', __('Pixel Primary Menu', 'pixel'));
        register_nav_menu('secondary', __('Pixel Footer Menu', 'pixel'));
    endif;
...
function pixel_widgets_setup() {
    global $pixel_options;
    // Widget areas
    if (function_exists('register_sidebar')) :
        // Sidebar right
        register_sidebar(array(
            'name' => __("Post Sidebar Widget Here", 'pixel'),
            'id' => "pixel-post-sidebar",
        ...
        register_sidebar(array(
         ...
         register_sidebar(array(
            ...
            'id' => "pixel-banner-sidebar",
            'description' => __('Sidebar for Banner posts', 'pixel'),
...
add_action('widgets_init', 'pixel_widgets_setup');
// The excerpt "more" button
function pixel_excerpt($text) {
    return str_replace('[&hellip;]', '[&hellip;]<a class="" title="'. sprintf (__('Read more on %s','pixel'), get_the_title()).'" href="'.get_permalink().'">' . __(' Read more','pixel') . '</a>', $text);
}
add_filter('the_excerpt', 'pixel_excerpt');
// wp_title filter
function pixel_title($output) {
    echo $output;
    // Add the blog name
    bloginfo('name');
    ...
    if (!empty($paged) && ($paged >= 2 || $page >= 2)) echo ' - ' . sprintf(__('Page %s', 'pixel'), max($paged, $page));
}
add_filter('wp_title', 'pixel_title');
 ...
 */
function pixel_load_theme_assets() {
    global $pixel_options;
    wp_enqueue_style( 'pixel-font-sans', 'https://fonts.googleapis.com/css?family=Source+Sans+Pro:600" rel="stylesheet', '', '' );
    wp_enqueue_style( 'pixel-font-play', 'https://fonts.googleapis.com/css?family=Playfair+Display:400,700" rel="stylesheet', '', '' );
    wp_enqueue_style( 'pixel-fontawesome', 'https://opensource.keycdn.com/fontawesome/4.6.3/font-awesome.min.css' , '', '' );
    // Enqueue all the theme CSS  
    wp_enqueue_style('pixel-bootstrap', get_template_directory_uri().'/assets/css/bootstrap.css '); 
    wp_enqueue_style('pixel-plugin', get_template_directory_uri().'/assets/css/plugins.min.css');
    wp_enqueue_style('pixel-main', get_template_directory_uri().'/assets/css/style.css');
    wp_enqueue_style( 'pixel-style', get_stylesheet_uri() );
    // Enqueue all the js files of theme
    wp_enqueue_script('pixel_jquery', get_template_directory_uri().'/assets/js/jquery-1.11.1.min.js', array(), FALSE, TRUE);
    wp_enqueue_script('pixel_bootstrap', get_template_directory_uri().'/assets/js/bootstrap.min.js', array(), FALSE, TRUE);
    wp_enqueue_script('pixel_plugins', get_template_directory_uri().'/assets/js/plugins.min.js', array(), FALSE, TRUE);
    wp_enqueue_script('pixel_jquery-main', get_template_directory_uri().'/assets/js/main.js', array(), FALSE, TRUE);
    // custom css append code here
    $inline_css='';
    if(isset($pixel_options['extra-css'])){
    $inline_css.=$pixel_options['extra-css'];
    }
    wp_add_inline_style( 'pixel-style', $inline_css );
    if(isset($pixel_options['typography-body']['font-family']) && $pixel_options['typography-body']['font-family']!=''&& $pixel_options['typography-body']['font-weight']!='') {
    wp_enqueue_style('googlefont-custom', 'http://fonts.googleapis.com/css?family='.esc_attr($pixel_options['typography-body']['font-family']));
    }
    if(isset($pixel_options['typography-h1']['font-family']) && $pixel_options['typography-h1']['font-family']!=''&& $pixel_options['typography-h1']['font-weight']!='') {
    wp_enqueue_style('googlefont-h1', 'http://fonts.googleapis.com/css?family='.esc_attr($pixel_options['typography-h1']['font-family']));
    }
    if(isset($pixel_options['typography-h2']['font-family']) && $pixel_options['typography-h2']['font-family']!=''&& $pixel_options['typography-h2']['font-weight']!='') {
    wp_enqueue_style('googlefont-h2', 'http://fonts.googleapis.com/css?family='.esc_attr($pixel_options['typography-h2']['font-family']));
    }
    if(isset($pixel_options['typography-h3']['font-family']) && $pixel_options['typography-h3']['font-family']!=''&& $pixel_options['typography-h3']['font-weight']!='') {
    wp_enqueue_style('googlefont-h3', 'http://fonts.googleapis.com/css?family='.esc_attr($pixel_options['typography-h3']['font-family']));
    }
    if(isset($pixel_options['typography-h4']['font-family']) && $pixel_options['typography-h4']['font-family']!=''&& $pixel_options['typography-h4']['font-weight']!='') {
    wp_enqueue_style('googlefont-h4', 'http://fonts.googleapis.com/css?family='.esc_attr($pixel_options['typography-h4']['font-family']));
    }
    if(isset($pixel_options['typography-h5']['font-family']) && $pixel_options['typography-h5']['font-family']!=''&& $pixel_options['typography-h5']['font-weight']!='') {
    wp_enqueue_style('googlefont-h5', 'http://fonts.googleapis.com/css?family='.$pixel_options['typography-h5']['font-family']);
    }
    if(isset($pixel_options['typography-h6']['font-family']) && $pixel_options['typography-h6']['font-family']!=''&& $pixel_options['typography-h6']['font-weight']!='') {
    wp_enqueue_style('googlefont-h6', 'http://fonts.googleapis.com/css?family='.$pixel_options['typography-h6']['font-family']);
    }
    // theme color variation code here
    $color_variation ='';
    if(isset($pixel_options['custom_color_primary']) && $pixel_options['custom_color_primary']!=''){
    $main_custom_color_primary= esc_attr($pixel_options['custom_color_primary']);
    } else {
    $main_custom_color_primary= "#C427F7";
    }
    if(isset($pixel_options['custom_color_hover']) && $pixel_options['custom_color_hover']!=''){
    $main_custom_color_hover= esc_attr($pixel_options['custom_color_hover']);
    } else {
    ...
    $color_variation='
        ...
        .tbeer-featured-news-column .tbeer-featured-news .tbeer-news-details a.tbeer-category-meta,
        .tbeer-latest-article .tbeer-latest-article-details a.tbeer-category-meta,
        .tbeer-btn,
        ...
        .tbeer-search-result-section .tbeer-single-post-wrapper .tbeer-main-content .tbeer-search-result .tbeer-search-result-details .tbeer-news-post-meta a:hover:before,div.tbeer-category-meta a   {
          background-color: '.$main_custom_color_primary.';
        }
        ...
        .tbeer-featured-news-column .tbeer-featured-news .tbeer-news-details h3 a:hover,
        .tbeer-featured-news-column .tbeer-featured-news .tbeer-news-details .tbeer-news-post-meta a:hover,
        .tbeer-featured-news-column .tbeer-featured-news .tbeer-news-details .tbeer-news-post-meta a:hover:before,
    ...
    wp_add_inline_style( 'pixel-style', $color_variation );
  }
add_action('wp_enqueue_scripts', 'pixel_load_theme_assets');
// Enqueue comment-reply script if comments_open and singular
function pixel_enqueue_comment_reply() {
        if ( is_singular() && comments_open() && get_option( 'thread_comments' ) ) {
                wp_enqueue_script( 'comment-reply' );
        }
}
add_action( 'wp_enqueue_scripts', 'pixel_enqueue_comment_reply' );
add_filter('nav_menu_css_class' , 'pixel_special_nav_class' , 10 , 2);
function pixel_special_nav_class($classes, $item){
...
add_action( 'tgmpa_register', 'pixel_register_required_plugins' );
function pixel_register_required_plugins() {
    $plugins = array(
        array(
            'name'      => 'CMB2',
            'slug'       => 'cmb2',
    ...
    $config = array(
        ...
        'strings'      => array(
            'page_title'                      => __( 'Install Required Plugins', 'pixel' ),
            'menu_title'                      => __( 'Install Plugins', 'pixel' ),
            'installing'                      => __( 'Installing Plugin: %s', 'pixel' ), // %s = plugin name.
            'oops'                            => __( 'Something went wrong with the plugin API.', 'pixel' ),
            'notice_can_install_required'     => _n_noop( 'This theme requires the following plugin: %1$s.', 'This theme requires the following plugins: %1$s.' , 'pixel'), // %1$s = plugin name(s).
            'notice_can_install_recommended'  => _n_noop( 'This theme recommends the following plugin: %1$s.', 'This theme recommends the following plugins: %1$s.', 'pixel' ), // %1$s = plugin name(s).
            'notice_cannot_install'           => _n_noop( 'Sorry, but you do not have the correct permissions to install the %s plugin. Contact the administrator of this site for help on getting the plugin installed.', 'Sorry, but you do not have the correct permissions to install the %s plugins. Contact the administrator of this site for help on getting the plugins installed.', 'pixel' ), // %1$s = plugin name(s).
            'notice_can_activate_required'    => _n_noop( 'The following required plugin is currently inactive: %1$s.', 'The following required plugins are currently inactive: %1$s.', 'pixel' ), // %1$s = plugin name(s).
            'notice_can_activate_recommended' => _n_noop( 'The following recommended plugin is currently inactive: %1$s.', 'The following recommended plugins are currently inactive: %1$s.', 'pixel' ), // %1$s = plugin name(s).
            'notice_cannot_activate'          => _n_noop( 'Sorry, but you do not have the correct permissions to activate the %s plugin. Contact the administrator of this site for help on getting the plugin activated.', 'Sorry, but you do not have the correct permissions to activate the %s plugins. Contact the administrator of this site for help on getting the plugins activated.', 'pixel' ), // %1$s = plugin name(s).
            'notice_ask_to_update'            => _n_noop( 'The following plugin needs to be updated to its latest version to ensure maximum compatibility with this theme: %1$s.', 'The following plugins need to be updated to their latest version to ensure maximum compatibility with this theme: %1$s.', 'pixel' ), // %1$s = plugin name(s).
            'notice_cannot_update'            => _n_noop( 'Sorry, but you do not have the correct permissions to update the %s plugin. Contact the administrator of this site for help on getting the plugin updated.', 'Sorry, but you do not have the correct permissions to update the %s plugins. Contact the administrator of this site for help on getting the plugins updated.' , 'pixel'), // %1$s = plugin name(s).
            'install_link'                    => _n_noop( 'Begin installing plugin', 'Begin installing plugins', 'pixel' ),
            'activate_link'                   => _n_noop( 'Begin activating plugin', 'Begin activating plugins', 'pixel' ),
            'return'                          => __( 'Return to Required Plugins Installer', 'pixel' ),
            'plugin_activated'                => __( 'Plugin activated successfully.', 'pixel' ),
            'complete'                        => __( 'All plugins installed and activated successfully. %s', 'pixel' ), // %s = dashboard link.
            'nag_type'                        => 'updated' // Determines admin notice type - can only be 'updated', 'update-nag' or 'error'.
        )
    );
    tgmpa( $plugins, $config );
}
function pixel_comment($comment, $args, $depth) {
        ...
        <?php if($depth>1): echo '<div class="media">'; else : echo'<div class="media-body">'; endif;?>
            <h3><a href="#!"><?php echo get_comment_author_link(); ?></a>
                <span class="tbeer-comment-time"><?php
                            ...
                            echo human_time_diff( get_the_time('U'), current_time('timestamp') ) . ' ago'; ?></a><?php edit_comment_link( __( '(Edit)','pixel' ), '  ', '' );
                        ?>
                </span>
            </h3>
            ...
            <?php if ( $comment->comment_approved == '0' ) : ?>
                <em class="comment-awaiting-moderation"><?php _e( 'Your comment is awaiting moderation.','pixel' ); ?></em>
                <br />
            <?php endif; ?>
            </div>
        </div>
...
/*====================================*\
    POPULAR POSTS
\*====================================*/
function pixel_getPostViews($postID){
...
function pixel_setPostViews($postID) {
    $count_key = 'post_views_count';
    $count = get_post_meta($postID, $count_key, true);
    if ($count == ''){
        $count = 0;
...
//For excerpt
function pixel_excerpt_more( $more ) {
    return '...';
}
add_filter('excerpt_more', 'pixel_excerpt_more');
function pixel_excerpt_length( $length ) {
    return 25;
}
add_filter( 'excerpt_length', 'pixel_excerpt_length', 999 );
// cmb2
function pixel_post_meta() {
...
add_action( 'cmb2_admin_init', 'pixel_post_meta' );
// pixel leak comment arrange
add_filter( 'comment_form_fields', 'pixel_move_comment_field' );
function pixel_move_comment_field( $fields ) {
    $comment_field = $fields['comment'];
    unset( $fields['comment'] );
    $fields['comment'] = $comment_field;
    return $fields;
}
if ( ! function_exists( 'pixel_load_widgets' ) ) :
    /**
     * Load widgets.
     *
     * @since 1.0.0
     */
    function pixel_load_widgets() {
        // Advanced Recent Posts widget.
        register_widget( 'Pixel_Recent_Posts_Widget' );
    }
endif;
add_action( 'widgets_init', 'pixel_load_widgets' );
if ( ! class_exists( 'Pixel_Recent_Posts_Widget' ) ) :
    /**
     * Advanced Recent Posts Widget Class
     *
     * @since 1.0.0
     */
    class Pixel_Recent_Posts_Widget extends WP_Widget {
        /**
         * Constructor.
         *
         * @since 1.0.0
        ...
        function __construct() {
            $opts = array(
                'classname'   => 'pixel_advanced_recent_posts',
                'description' => __( 'Featured Post Widget. Displays recent posts with thumbnail.', 'pixel' ),
            );
            parent::__construct( 'pixel-advanced-recent-posts', __( 'Pixel: Featured Posts', 'pixel' ), $opts );
        }
        /**
        ...
        function form( $instance ) {
            ...
            ?>
            <p>
                <label for="<?php echo esc_attr( $this->get_field_id( 'post_category' ) ); ?>"><?php _e( 'Select Category:', 'pixel' ); ?></label>
                <?php
                $cat_args = array(
                    'orderby'         => 'name',
                    'hide_empty'      => 0,
                    ...
                    'show_option_all' => __( 'All Categories','pixel' ),
                );
...
// excerpt
function Pixel_the_excerpt_max_charlength($charlength) {
...
if ( ! function_exists( 'Pixel_custom_category' ) ) :
    /**
     * Load widgets.
     *
     * @since 1.0.0
     */
    function Pixel_custom_category() {
        // Advanced Recent Posts widget.
        register_widget( 'Pixel_custom_category_widget' );
    }
endif;
add_action( 'widgets_init', 'Pixel_custom_category' );
if ( ! class_exists( 'Pixel_custom_category_widget' ) ) :
    /**
     * Advanced Recent Posts Widget Class
     *
     * @since 1.0.0
     */
    class Pixel_custom_category_widget extends WP_Widget {
        /**
         * Constructor.
         *
         * @since 1.0.0
        ...
        function __construct() {
            $opts = array(
                'classname'   => 'Pixel_custom_category_widget',
                'description' => __( 'Displays category name with their subcategories.', 'pixel' ),
            );
            parent::__construct( 'pixel-custom-category-widget', __( 'Pixel: Custom Category', 'pixel' ), $opts );
        }
        /**
         * Echo the widget content.
         *
